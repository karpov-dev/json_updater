...
def find_usages(json_data, path):
    if type(json_data) is dict:
        for key in json_data:
            json_data[key] = find_usages(json_data[key], key)
...
DEBUG_MODE = False
if DEBUG_MODE:
    UPDATE_DICT = {'Kate Client': 'THE BACKGROUND', 'test': 'TEST'}
    AUTO_UPDATE = True
    ...
    parser.add_argument('--update_dict', help='word_to_change:new_word,word_to_change:new_word and etc.')
    parser.add_argument('--auto_update', help='True or False. Enable Auto change words or manual changing. '
                                              'Default - manual')
...
with open(FILE_DIR + '/' + 'updated_' + FILE_NAME, 'w') as file:
